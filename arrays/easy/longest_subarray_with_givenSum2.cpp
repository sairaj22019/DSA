// given an array(positives and negatives) and number k. find length of longest subarray whose sum is k
...
// optimal(hashing)  TC-O(nlogn{map} or n{unoredered map} or n^2{worst case in unoderered map})
//                  SC-O(n)
int longestSubarray(vector<int>& a,int k){
    int n= a.size();
